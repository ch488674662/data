...
def read_data(file_path, id, name):
    ...
    return output
#关系的训练集
train_interrelation = pd.read_csv('Train_Interrelation.csv', dtype=str)
print(len(train_interrelation))
#需求的训练集
Train_Achievements = read_data('Train_Achievements.csv', 'Aid', 'Achievements')
print(len(Train_Achievements))
#技术需求表
Requirements = read_data('Requirements.csv', 'Rid', 'Requirements')
print(len(Requirements))
#测试集中需要预测的对应项，需求文档和成果文档是什么关系
TestPrediction = pd.read_csv('TestPrediction.csv', dtype=str)
print(len(TestPrediction))
#测试集中的需求文档
Test_Achievements = read_data('Test_Achievements.csv', 'Aid', 'Achievements')
